...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setGameState(prev => {
      ...
      chessLogicInstance, gameState.selectedSquare, gameState.validMoves,
      isGameInProgress, gameState.isCheckmate, gameState.isStalemate, gameState.winner
    ]);
  ...
  useEffect(() => {
    ...
    if (isAITurn && !gameState.isCheckmate && !gameState.isStalemate && isGameInProgress) {
        ...
        if (validMovesUci.length === 0) {
            ...
            if (isKingInCheckForAI) setGameState(prev => ({...prev, isCheckmate: true, winner: PieceColor.WHITE, gameStatusMessage: "Checkmate! White wins."}));
            else setGameState(prev => ({...prev, isStalemate: true, gameStatusMessage: "Stalemate! It's a draw."}));
            setIsAITurn(false); return;
        }
        ...
        timeoutId = window.setTimeout(async () => {
            if (gameState.isCheckmate || gameState.isStalemate || !isGameInProgress || !isAITurn) {
                if (isAITurn) setIsAITurn(false); return;
            }
            ...
            if (!aiChosenUci || !validMovesUci.includes(aiChosenUci)) {
                 if (validMovesUci.length > 0) { 
                    ...
                    console.error("AI Turn Critical: No valid moves available."); setIsAITurn(false); 
                     const isKingInCheckForAI = chessLogicInstance.isKingInCheck(aiPlayerColor);
                     if (isKingInCheckForAI && !gameState.isCheckmate) setGameState(prev => ({ ...prev, isCheckmate: true, winner: PieceColor.WHITE, gameStatusMessage: "Checkmate! White wins (emergency)." }));
                     else if (!isKingInCheckForAI && !gameState.isStalemate) setGameState(prev => ({ ...prev, isStalemate: true, gameStatusMessage: "Stalemate! (emergency)." }));
                    return;
                }
            }
            if (gameState.isCheckmate || gameState.isStalemate || !isGameInProgress || !isAITurn) {
            ...
            if (aiChosenUci) { 
              ...
              if (promotionChar) {
                switch (promotionChar) {
                  case 'q': promotionPiece = PieceType.QUEEN; break; case 'r': promotionPiece = PieceType.ROOK; break;
                  case 'b': promotionPiece = PieceType.BISHOP; break; case 'n': promotionPiece = PieceType.KNIGHT; break;
                }
              }
              performMove({ from, to, promotion: promotionPiece });
            } else { console.error("AI failed to select a move."); setIsAITurn(false); }
        }, aiThinkTime);
    }
  ...
  return (
    ...
    <Head>
        ...
        {/* Specific meta tags for this page can go here if different from _app.tsx */}
    </Head>
    <div className="min-h-screen bg-slate-800 flex flex-col items-center justify-center p-2 sm:p-4 selection:bg-emerald-500 selection:text-white">
      ...
      <main className="flex flex-col md:flex-row items-center md:items-start gap-4 sm:gap-6 w-full max-w-4xl mx-auto">
        {/* Grouping div for Header and Board */}
        ...
        {/* Controls Panel */}
        <div className="w-full mt-4 md:mt-0 md:ml-6 md:flex-1 space-y-4 min-w-[240px] sm:min-w-[280px]">
      ...
      {showCookieBanner && (
        <CookieBanner
          ...
          onLearnMore={() => { /* Navigation handled by Link in banner for Next.js */ }}
        />
      )}
